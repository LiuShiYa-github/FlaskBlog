...
def user_del(user_id):
    ...
    if tag:
        ...
        return redirect(url_for('admin.user'))
```
到这里我们所有的关于基本的几个管理功能的逻辑就完成了，当然修改密码还有很多实现方法以及逻辑，到这里我们已经掌握了基本的所有内容，大家就可以按照自己的思路去实现这些功能，不一定非要与教程中保持一致！

# 实现全站导航菜单及首页数据展示
```text
Flask上下文处理器我们博客的设计是全站顶部菜单为通用菜单，调用的数据为文章的分类，那么如何将文章分类数据传递到每个页面的上下文呢？为了将新变量自动注入模板的上下文中，Flask 中存在上下文处理器context_processor。上下文处理器在呈现模板之前运行，并且能够将新值注入模板上下文。上下文处理器是返回字典的函数。
```
## Flask上下文处理器
...
{% endblock member %}
```
这里我们仅仅实现了一个前端样式，有些字段我们并没有创建，这一部分内容也留给大家当做作业去完成！
